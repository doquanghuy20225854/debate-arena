...
interface RaisedHandsListProps {
  ...
  sortBy?: 'latest' | 'upvotes';
  onSortChange?: (sortBy: 'latest' | 'upvotes') => void;
}
...
const RaisedHandsList: React.FC<RaisedHandsListProps> = ({
  ...
  const [sortedHands, setSortedHands] = useState<RaisedHand[]>([]);
  ...
  const handleUpvote = (questionId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    if (onUpvote) {
      onUpvote(questionId);
      // TODO: Socket.IO emit
